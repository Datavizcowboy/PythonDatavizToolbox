...
folder = './' 
target = '/Users/jmartinez/Sites/TrueKMeans/Data_Contour/'
#dataset = 'UnequalVar'
#dataset = 'Mixture2D' 
...
#        distances = np.abs(points - centroids[:, np.newaxis]).sum(axis=2)
        ...
        # Cosine
#        distances = ((points * centroids[:, np.newaxis]).sum(axis=2))/np.sqrt(((points**2).sum)*((centroids[:]**2).sum))
        # Max Min
#        distances = (np.min(points,centroids[:, np.newaxis]).sum(axis=2))/(np.max(points,centroids[:, np.newaxis]).sum(axis=2))
